...
export function parseAttributes(context: Context, svgnode: SvgNode, node?: HTMLElement): Context {
  ...
  const visibility = getAttribute(domNode, 'visibility', context.styleSheets)
  if (visibility) {
    context.attributeState.visibility = visibility
  }
  // fill mode
  const fillColor = getAttribute(domNode, 'fill', context.styleSheets)
  if (fillColor) {
    context.attributeState.fill = getFill(fillColor, context, svgnode)
  }

  // opacity is realized via a pdf graphics state
  const fillOpacity = getAttribute(domNode, 'fill-opacity', context.styleSheets)
  if (fillOpacity) {
    context.attributeState.fillOpacity = parseFloat(fillOpacity)
  }
  const strokeOpacity = getAttribute(domNode, 'stroke-opacity', context.styleSheets)
  if (strokeOpacity) {
    context.attributeState.strokeOpacity = parseFloat(strokeOpacity)
  }
  const opacity = getAttribute(domNode, 'opacity', context.styleSheets)
  if (opacity) {
    context.attributeState.opacity = parseFloat(opacity)
  }

  // stroke mode
  let strokeWidth: any = getAttribute(domNode, 'stroke-width', context.styleSheets)
  ...
  const strokeColor = getAttribute(domNode, 'stroke', context.styleSheets)
  if (strokeColor) {
    if (strokeColor === 'none') {
  ...
  const lineCap = getAttribute(domNode, 'stroke-linecap', context.styleSheets)
  if (lineCap) {
    context.attributeState.strokeLinecap = lineCap
  }
  const lineJoin = getAttribute(domNode, 'stroke-linejoin', context.styleSheets)
  if (lineJoin) {
    context.attributeState.strokeLinejoin = lineJoin
  }
  let dashArray: any = getAttribute(domNode, 'stroke-dasharray', context.styleSheets)
  if (dashArray) {
    dashArray = parseFloats(dashArray)
    const dashOffset = parseInt(getAttribute(domNode, 'stroke-dashoffset', context.styleSheets)) || 0
    context.attributeState.strokeDasharray = dashArray
    context.attributeState.strokeDashoffset = dashOffset
  }
  const miterLimit = getAttribute(domNode, 'stroke-miterlimit', context.styleSheets)
  ...
  const fontWeight = getAttribute(domNode, 'font-weight', context.styleSheets)
  if (fontWeight) {
    context.attributeState.fontWeight = fontWeight
  }

  const fontStyle = getAttribute(domNode, 'font-style', context.styleSheets)
  if (fontStyle) {
    context.attributeState.fontStyle = fontStyle
  }

  const fontFamily = getAttribute(domNode, 'font-family', context.styleSheets)
  if (fontFamily) {
  ...
  const fontSize = getAttribute(domNode, 'font-size', context.styleSheets)
  if (fontSize) {
    context.attributeState.fontSize = parseFloat(fontSize)
  }

  const alignmentBaseline =
    getAttribute(domNode, 'vertical-align', context.styleSheets) || getAttribute(domNode, 'alignment-baseline', context.styleSheets)
  ...
  const textAnchor = getAttribute(domNode, 'text-anchor', context.styleSheets)
  if (textAnchor) {
