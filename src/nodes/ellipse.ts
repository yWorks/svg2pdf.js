...
export class Ellipse extends GeometryNode {
  ...
  constructor(node: HTMLElement, children: SvgNode[],context:Context) {
    super(node, children)
    this.rx = parseFloat(getAttribute(this.element, 'rx', context.styleSheets))
    this.ry = parseFloat(getAttribute(this.element, 'ry', context.styleSheets))
  }
  ...
  protected getPath(context: Context) {
    ...
    const x = parseFloat(getAttribute(this.element, 'cx', context.styleSheets)) || 0,
      y = parseFloat(getAttribute(this.element, 'cy', context.styleSheets)) || 0
