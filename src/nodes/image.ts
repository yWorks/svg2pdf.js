...
export class ImageNode extends GraphicsNode {
  protected renderCore(context: Context): void {
    ...
    const width = parseFloat(getAttribute(this.element, 'width', context.styleSheets)),
      height = parseFloat(getAttribute(this.element, 'height', context.styleSheets)),
      x = parseFloat(getAttribute(this.element, 'x', context.styleSheets)) || 0,
    ...
    if (dataUrl && dataUrl[2] === 'image/svg+xml') {
      ...
      const svgnode = parse(svgElement,context, idMap)
      svgnode.render(
