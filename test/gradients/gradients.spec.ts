...
describe('gradients', () => {
  it('gradient-default-coordinates', async () => {
    const { svgElement, width, height } = await loadSvg('/test/gradients/gradient-default-coordinates.svg')
  ...
  it('gradient-percent-offset', async () => {
    const { svgElement, width, height } = await loadSvg('/test/gradients/gradient-percent-offset.svg')
    const pdf = new jsPDF(width > height ? 'l' : 'p', 'pt', [width, height])
    await svg2pdf(svgElement, pdf, { loadExternalStyleSheets: true })
  ...
  it('gradient-stop-defaults', async () => {
    const { svgElement, width, height } = await loadSvg('/test/gradients/gradient-stop-defaults.svg')
    const pdf = new jsPDF(width > height ? 'l' : 'p', 'pt', [width, height])
    await svg2pdf(svgElement, pdf, { loadExternalStyleSheets: true })
